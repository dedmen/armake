...
/* In a future release of Bison, this section will be replaced
   by #include "rapify.tab.h".  */
#ifndef YY_YY_RAPIFY_TAB_H_INCLUDED
# define YY_YY_RAPIFY_TAB_H_INCLUDED
/* Debug traces.  */
#ifndef YYDEBUG
# define YYDEBUG 0
#endif
...
do                                                              \
  if (yychar == YYEMPTY)                                        \
    ...
    {                                                           \
      yyerror (result, lineref, YY_("syntax error: cannot back up")); \
      YYERROR;                                                  \
    }                                                           \
while (0)
  ...
  if (yydebug)                                                            \
    {                                                                     \
      ...
      yy_symbol_print (stderr,                                            \
                  Type, Value, Location, result, lineref); \
      YYFPRINTF (stderr, "\n");                                           \
    }                                                                     \
} while (0)
...
yy_symbol_print (FILE *yyoutput, int yytype, YYSTYPE const * const yyvaluep, YYLTYPE const * const yylocationp, struct class_ **result, struct lineref *lineref)
{
  ...
  YYFPRINTF (yyoutput, ": ");
  yy_symbol_value_print (yyoutput, yytype, yyvaluep, yylocationp, result, lineref);
  YYFPRINTF (yyoutput, ")");
}
  ...
  for (yyi = 0; yyi < yynrhs; yyi++)
    {
      ...
      yy_symbol_print (stderr,
                       ...
                       &(yyvsp[(yyi + 1) - (yynrhs)])
                       , &(yylsp[(yyi + 1) - (yynrhs)])                       , result, lineref);
      YYFPRINTF (stderr, "\n");
    }
}
...
do {                                    \
  if (yydebug)                          \
    yy_reduce_print (yyssp, yyvsp, yylsp, Rule, result, lineref); \
} while (0)
  ...
  if (!yyerrstatus)
    {
      ++yynerrs;
#if ! YYERROR_VERBOSE
      yyerror (result, lineref, YY_("syntax error"));
#else
      ...
      if (yychar <= YYEOF)
      ...
      else
        {
          yydestruct ("Error: discarding",
                      yytoken, &yylval, &yylloc, result, lineref);
          yychar = YYEMPTY;
        }
    }
  ...
  for (;;)
    {
      ...
      yydestruct ("Error: popping",
                  yystos[yystate], yyvsp, yylsp, result, lineref);
      YYPOPSTACK (1);
      yystate = *yyssp;
      YY_STACK_PRINT (yyss, yyssp);
    }
...
`-------------------------------------------------*/
yyexhaustedlab:
  yyerror (result, lineref, YY_("memory exhausted"));
  yyresult = 2;
  /* Fall through.  */
#endif
  ...
  while (yyssp != yyss)
    {
      yydestruct ("Cleanup: popping",
                  yystos[*yyssp], yyvsp, yylsp, result, lineref);
      YYPOPSTACK (1);
    }
#ifndef yyoverflow
  if (yyss != yyssa)
